...
| `docgen:read` | `list_gen_templates`, `get_gen_template`, `preview_gen_template`, `list_gen_documents`, `get_gen_document`, `list_gen_delivery_configs`, `list_gen_template_delivery_configs`, `list_gen_delivery_logs` |
...
Note the asymmetry worth warning users about: `delete_agreement` needs only `sign:write`,
but `void_agreement` needs `sign:send`. A token that can delete drafts may be unable to
cancel a sent agreement.
